...
- `script/test`
  - Validates the release-tool manifest before running tests.
...
- `script/install-zig`
  ...
  - Installs `cargo-zigbuild` from committed source and vendored dependencies with `cargo install --path --locked --offline`.
...
- `script/vendor-release-tools`
  ...
  - Generates/preserves the `cargo-zigbuild` lockfile, vendors its transitive crates into `vendor/release-tools/cargo-zigbuild/vendor/cache`, and writes `vendor/release-tools/manifest.toml`.
...
- `script/validate-release-tools`
  ...
  - Verifies manifest version consistency, artifact existence, SHA-256 checksums, `cargo-zigbuild` source/lock/vendor state, and release workflow/install-script network guardrails.
...
- `vendor/release-tools` should be generated by `script/vendor-release-tools`, not manually curated.
...
- Coverage changes: `script/test --cov` only when coverage tools are already installed.
- Dependency updates: `script/update`, then inspect `Cargo.lock` and `vendor/cache`, then rerun offline validation.
- Release-tool updates: `script/vendor-release-tools`, then inspect `vendor/release-tools`, run `script/validate-release-tools`, and run `script/install-zig` on a supported host.
